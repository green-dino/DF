...
class FileHandler:
    ...
    def list_files(directory):
        ...
        return [f for f in os.listdir(directory) if f.endswith(('.csv', '.xls', '.xlsx', '.doc', '.docx'))]
...
class DataFrameHandler:
    ...
    def read_file_to_dataframe(file_path, sheet_name=None):
        ...
        if file_path.endswith('.csv'):
            ...
            raise ValueError("Unsupported file format. Please provide a CSV, XLS/XLSX, DOC, or DOCX file.")
...
def get_directory():
    ...
    while True:
        ...
        if not files:
            print("No CSV, XLS/XLSX, DOC, or DOCX files found in the specified directory.")
            continue
...
def process_file(file_dir, files):
    ...
    dataframe = DataFrameHandler.read_file_to_dataframe(file_path)
